...
const Display = ({ closed, locked }) => {
  ...
  return (
    <div className="display panel" data-testid="display-component">
      <div data-testid="gate-lock-status-display" className={lockedClass}>{locked ? 'Locked' : 'Unlocked'}</div>
      <div data-testid = "gate-status-display" className={closedClass}>{closed ? 'Closed' : 'Open'}</div>
    </div>
  );
};
